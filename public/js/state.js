...
export const state = {
  ...
  opStatusUnsub: null, // opStatus/current listener (op-status.js)
  allJoinRequests: [],
  allPlayers: [],
  ...
  characterDeckAbilityTab: 'active',     // Phase 14 S15 -- character deck viewer: which Abilities sub-tab is showing ('active' | 'vault' | 'experience' | 'beastforms' | 'stances'); shared across GM/player views since only one deck is ever open at a time
  ...
  detailEditDraft: null,   // { name, category, ancestry, aliases, date, parentId, tags, relatedIds, pendingRelatedSlugs } — in-progress entity edit; re-populates edit inputs across re-renders so unrelated snapshot updates don't clobber typing
  ...
  currentSceneId: null,  // from config/campaign doc, GM-selected Scene entity for opening message
